...
async def test_add_and_remove_like_sequence(db_session: AsyncSession):
    ...
    # STEP 2: Insert a dummy event (foreign key references user.id)
    event = Events(
        title="Like Event",
    ...
    # STEP 3a: First add_like(...) should succeed
    ...
    # STEP 3b: Second add_like(...) of the same pair → should return False
    second_add = await ctrl.add_like(payload)
    assert second_add is False, "A duplicate add_like(...) must return False"

    # STEP 4: Exactly one row is in the table, so remove_like(...) succeeds once...
    ...
    # STEP 5: Now no row remains, so a second remove_like(...) returns False
...
async def test_remove_like_when_none_exists(db_session: AsyncSession):
    ...
    """
    # STEP 1: Insert one dummy user + one dummy event (no calls to add_like)
    user = Users(
    ...
    # Immediately calling remove_like(...) on a pair that was never inserted yields False
    payload = LikeBase(user_id=user.id, event_id=event.id)
...
async def test_get_likes_for_user_returns_exactly_one_row(db_session: AsyncSession):
    ...
    # 2) Insert exactly one like via the controller
    payload = LikeBase(user_id=user.id, event_id=event.id)
    assert await ctrl.add_like(payload) is True

    # 3) Now retrieve via get_likes_for_user(...)
    ...
    # check the two foreign‐key columns explicitly
